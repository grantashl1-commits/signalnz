...
- **Reactions / replies / mentions / edits**: `message_reactions` table + `community_messages.reply_to_id`, `mentions[]` (uuid[]), `edited_at`, `original_content`. RLS allows author edits within 15 minutes. Mentions surface a member-picker on `@` and highlight messages for the mentioned user.
- **Unread badges (`Community.tsx`)**: `community_last_reads` (one row per user+group, last_read_at) is upserted by `ChatRoom.tsx` whenever messages render. `Community.tsx` computes per-group unread by counting `community_messages` where `created_at > last_read_at` and `user_id != me` and `is_removed = false`. Badges appear on each group tab in the chat sidebar and a total appears on the Chat tab itself. A realtime INSERT subscription (`community-unread-watch`) bumps the count live for any joined group that isn't currently active.
- **Real Nearby presence (`NearbyView.tsx`)**: per-suburb presence channel `nearby-presence-{suburb-slug}`. Online dots and "active now" count derive from `presenceState()` — no `Math.random()`. Each Nearby user card has a "Message" button that calls `onMessageMember(userId, displayName)` → `Community.tsx` switches to the Chat tab and passes `initialDmUserId` to `ChatRoom`, which prefills `@FirstName ` in the composer.
