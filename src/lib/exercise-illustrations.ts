...
export async function findIllustrationUrl(name: string): Promise<string | null> {
  // 1. Check the local watercolour set in /public/images/exercises/<slug>.png.
  //    Synchronous, no network — if it's there we use it.
  const localUrl = getExerciseImageByName(name);
  if (localUrl) return localUrl;

  // 2. Fall back to the Supabase storage index (illustrations generated by
  //    the edge function or seeded by Lovable).
