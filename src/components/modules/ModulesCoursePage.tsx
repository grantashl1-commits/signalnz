...
import { MODULES_COURSE } from "@/data/modules-course";
...
const TOTAL_LESSONS = MODULES_COURSE.reduce((s, m) => s + m.lessons.length, 0);

export default function ModulesCoursePage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    (supabase.from("connect_course_progress" as any).select("activity_id, lesson_id, completed")
      ...
      .then(({ data }: { data: any[] | null }) => {
        ...
        // Filter to only Learning-module activities
        const modActs = new Set([...acts].filter(id => typeof id === "string" && id.startsWith("mod-")));
        setCompletedActivities(modActs);
        const lessonDone = new Set<string>();
        MODULES_COURSE.forEach(m => m.lessons.forEach(l => {
          if (l.activities.every(a => modActs.has(a.id))) lessonDone.add(l.id);
        }));
  ...
  const handleSaveProgress = useCallback(async (activityId: string, response: any, saveToVaultFlag: boolean) => {
    ...
    if (saveToVaultFlag && hasJournalAccess) {
      ...
      const vaultEntry: VaultEntry = {
        id: `modules-${activityId}-${Date.now()}`,
        entryId: activityId,
        category: "modules-course",
  ...
  const totalProgress = useMemo(() => {
    const total = MODULES_COURSE.reduce((s, m) =>
        ...
        <AtmosphericHero size="sm">
          ...
          <div className="text-center relative z-10">
            ...
            <p className="font-body text-sm text-primary-foreground/50 mt-2">
              {MODULES_COURSE.length} modules · {TOTAL_LESSONS} lessons · rooted in your cycle
            </p>
            {totalProgress > 0 && (
            ...
            {view === "modules" && (
              <motion.div key="modules" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="grid gap-3">
                  {MODULES_COURSE.map((mod, i) => {
                    const progress = getModuleProgress(mod);
                    const isUnlocked = i === 0 || getModuleProgress(MODULES_COURSE[i - 1]) >= 60;
                    return (
