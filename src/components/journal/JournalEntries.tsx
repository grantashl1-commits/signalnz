...
import { ArrowLeft, Loader2, Search, Plus, X, BookOpen } from "lucide-react";
...
// ── Entry Types ───────────────────────────────────────────────
const ENTRY_TYPES = [
  { key: "free-write", label: "Free Write", desc: "Write whatever is on your mind" },
  { key: "guided-prompt", label: "Guided Prompt", desc: "Use reflective prompts" },
  { key: "tiny-win", label: "Tiny Win", desc: "Celebrate a small victory" },
  { key: "funny-moment", label: "Funny Moment", desc: "Capture something that made you laugh" },
  { key: "gratitude-note", label: "Gratitude Note", desc: "Acknowledge what you are grateful for" },
  { key: "letter-future-self", label: "Letter to Future Self", desc: "Write to who you are becoming" },
  { key: "dream-entry", label: "Dream Entry", desc: "Record a dream or aspiration" },
  { key: "lesson-learned", label: "Lesson Learned", desc: "Reflect on something you discovered" },
];
...
function groupEntries(entries: JournalEntry[]) {
  const now = new Date();
  const todayStr = now.toLocaleDateString("en-NZ", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
  const yesterday = new Date(now);
  yesterday.setDate(yesterday.getDate() - 1);
  const yesterdayStr = yesterday.toLocaleDateString("en-NZ", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
  ...
  const groups: { label: string; entries: JournalEntry[] }[] = [
    ...
    { label: "Earlier Chapters", entries: [] },
  ];

  for (const e of entries) {
    if (e.date === todayStr) groups[0].entries.push(e);
    else if (e.date === yesterdayStr) groups[1].entries.push(e);
    else if (e.timestamp > weekAgo) groups[2].entries.push(e);
...
function NewEntryForm({
  ...
  // For specific entry types, show a single textarea
  const isSinglePrompt = ["free-write", "tiny-win", "funny-moment", "gratitude-note", "letter-future-self", "dream-entry", "lesson-learned"].includes(entryType);

  const singlePromptConfig: Record<string, { label: string; ph: string }> = {
    ...
    "funny-moment": { label: "Something that made me laugh...", ph: "Capture the joy. What was the moment that made you smile or laugh today?" },
    "gratitude-note": { label: "I am grateful for...", ph: "What people, moments, or small things brought a sense of thankfulness today?" },
  ...
  const save = () => {
    setSaving(true);
    haptic("medium");
    const title = isSinglePrompt
      ? (prompts["main"] || "").slice(0, 60) || typeConfig?.label || "Untitled"
      : today;
  ...
  return (
    <div className="space-y-3 pb-10">
      ...
      {isSinglePrompt ? (
        <div className="card-warm p-5">
          <label className="block font-display text-lg italic text-foreground mb-2.5">
            {singlePromptConfig[entryType]?.label || "Write..."}
          </label>
          <textarea
            rows={8}
            value={prompts["main"] || ""}
            onChange={(e) => setPrompts((prev) => ({ ...prev, main: e.target.value }))}
            placeholder={singlePromptConfig[entryType]?.ph || "Start writing..."}
...
function AnalysisView({ entry, onBack, isMilestone }: { entry: JournalEntry; onBack: () => void; isMilestone?: boolean }) {
  ...
  return (
    <div className="pb-10">
      ...
      {ai.ifs_insight && (
        <Section title="Parts Work Insight (IFS)" accent="hsl(var(--secondary))">
        ...
        <Section title="Patterns I Notice">
          {ai.patterns.map((p: string, i: number) => (
            <div key={i} className="flex gap-2.5 mb-2.5">
              <span className="text-sm text-primary flex-shrink-0 mt-0.5">·</span>
...
export default function JournalEntries({
  ...
  const handleMilestoneAnalyse = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("journal-ai", {
        body: { milestoneType: nextMilestone.type, milestoneCount: nextMilestone.count, milestoneLabel: nextMilestone.label, allEntries: entries.slice(0, nextMilestone.count) },
      });
      if (error) throw error;
  ...
  // Analysis / milestone views
  if (view === "analysis" && activeEntry) {
    return <AnalysisView entry={activeEntry} onBack={() => setView("list")} />;
  }
  ...
  // Entry type picker
  if (view === "pick-type") {
    return <EntryTypePicker onSelect={(t) => { setEntryType(t); setView("new"); }} onCancel={() => setView("list")} />;
  }

  // New entry form
  if (view === "new") {
    return <NewEntryForm entryType={entryType} onSaved={(updated) => { setEntries(updated); setView("list"); }} onCancel={() => setView("list")} />;
  }
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <button
        ...
        className="touch-btn w-full card-warm p-4 flex items-center gap-3 active:opacity-90"
      >
        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
          <Plus className="h-5 w-5 text-primary" />
        </div>
        <div className="text-left">
          <p className="font-display text-[15px] italic text-foreground">New entry</p>
          <p className="font-mono text-[11px] text-muted-foreground">Free write, gratitude, tiny win, and more</p>
        </div>
      </button>
      ...
      {nextMilestone && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-gradient-to-br from-primary/8 to-phase-follicular/8 border border-primary/15 p-5">
          <div className="flex items-center gap-2 mb-2">
            <WildStar size={22} />
            <h3 className="font-display text-lg italic text-foreground">{nextMilestone.label}</h3>
          </div>
      ...
      {groups.map((group) => (
        <div key={group.label}>
          <p className="font-hand text-sm font-bold text-primary mb-2">{group.label}</p>
          <div className="space-y-3">
            {group.entries.map((e) => (
              <motion.div key={e.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="card-warm p-5">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1 min-w-0">
                    <p className="font-display text-base italic text-foreground mb-0.5 truncate">{e.title || e.date}</p>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-mono text-[11px] px-2 py-0.5 rounded-full bg-primary/10 text-primary">{getEntryTypeLabel(e.entryType)}</span>
                  ...
                  <div className="flex gap-1.5 flex-shrink-0 ml-2">
                    {onSaveToVault && (
                      <button onClick={() => onSaveToVault(e)} className="font-mono text-[10px] text-muted-foreground bg-secondary rounded-full px-2.5 py-1 active:opacity-70">
                        Save to vault
                      </button>
                    )}
                    {e.ai ? (
                      <button onClick={() => { setActiveEntry(e); setView("analysis"); }} className="font-mono text-[11px] text-muted-foreground bg-secondary rounded-full px-3 py-1.5 active:opacity-70">View analysis</button>
                    ) : (
                      <button onClick={() => handleAnalyse(e)} className="font-mono text-[11px] text-primary border border-primary/30 rounded-full px-3 py-1.5 active:opacity-70 flex items-center gap-1">
                        <HandDrawnSparkle size={12} color="hsl(var(--primary))" /> Analyse
                      </button>
                    )}
                  </div>
                </div>
                <p className="font-display text-[13px] italic text-muted-foreground line-clamp-2 leading-relaxed">{getPreview(e)}</p>
                {e.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {e.tags.map((t) => <Pill key={t} label={t} />)}
                  </div>
                )}
              </motion.div>
            ))}
