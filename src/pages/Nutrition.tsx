...
export default function NutritionPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 relative">
      {/* Background herbs */}
      <div className="absolute top-0 right-0 -translate-y-4 translate-x-4 pointer-events-none">
        <HerbCluster size={100} opacity={0.2} />
      </div>

      <div>
        <p className="font-hand text-sm font-bold text-primary">nutrition</p>
        <h1 className="font-display text-4xl font-bold italic text-foreground mt-1">Nutrition</h1>
        ...
        <div className="space-y-4">
          ...
          {todayMeals.map((meal, i) => (
            <motion.div key={meal.type} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
              className="relative overflow-hidden card-warm p-5"
            >
            ...
            <div className="flex gap-2 mb-2">
              {Array.from({ length: 8 }, (_, i) => (
                <button key={i} onClick={i === water ? addWater : undefined}
                  className={`h-7 w-7 rounded-full border transition-all ${i < water ? "bg-phase-follicular/30 border-phase-follicular/60" : "border-border hover:border-phase-follicular/30"}`}
                />
              ))}
            </div>
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] text-muted-foreground">{water}/8</span>
              <button onClick={addWater} disabled={water >= 8} className="rounded-full bg-phase-follicular/10 px-3 py-1 font-mono text-[10px] text-phase-follicular hover:bg-phase-follicular/20 disabled:opacity-30">+1</button>
            </div>
          </div>
      ...
      {activeTab === "recipes" && (
        <div className="space-y-6">
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setRecipePhaseFilter("all")} className={`rounded-full px-3 py-1.5 font-body text-xs font-medium transition-all ${recipePhaseFilter === "all" ? "bg-foreground text-background" : "bg-secondary text-muted-foreground"}`}>All</button>
            {(["menstrual", "follicular", "ovulatory", "luteal"] as Phase[]).map((phase) => (
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {filteredRecipes.map((recipe, i) => {
              ...
              return (
                ...
                >
                  {/* Botanical frame placeholder */}
                  <div className="h-24 bg-secondary/30 relative flex items-center justify-center overflow-hidden">
                    <CymatiSketch phase={recipe.phase} size={100} opacity={0.15} />
                    <p className="absolute font-hand text-sm text-muted-foreground italic">{recipe.name}</p>
                  </div>
                  ...
                  {expanded && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pb-4 border-t border-border pt-3 space-y-3">
                      ...
                      <BotanicalSprig width={120} opacity={0.15} />
                      <div>
